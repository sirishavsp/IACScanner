...
def generate_report(results, filepath):
    ...
    issue_counts = Counter(result['issue'] for result in sorted_results)

    # Save the counts to the database
    report = Report.query.get(filepath)
    if report is None:
        report = Report(id=filepath)
...
def pie_chart(filepath):
    report_db_entry = Report.query.filter_by(report_url=filepath).first()

    if report_db_entry is None or report_db_entry.issue_counts is None:
        return "Report not found", 404

    return render_template('view_pie_chart.html', issue_counts=report_db_entry.issue_counts)
